    ...
    <div class="row justify-content-center mf">
      <div class="col-lg-6 shadow-lg rounded-3 right-side">
        <form @submit.prevent="ChangeData()">
          <p class="bg-info text-primary title-form px-3 py-2 rounded-top">Update Supplier</p>
          <div class="row pf">
            <div class="col-3">
              <label for="NamaSupplier" class="form-label">Nama Supplier</label>
            </div>
            <div class="col-8">
              <input v-model="namaSupplier" type="text" class="form-control" id="NamaSupplier" name="namaSupplier" placeholder="Masukan Nama Supplier" autofocus />
            </div>
          </div>
          <div class="row pf">
            <div class="col-3">
            ...
            <div class="col-8">
              <input v-model="alamat" type="text" class="form-control" id="AlamatSupplier" name="alamat" placeholder="Masukan Alamat Supplier" autofocus />
            </div>
          </div>
          <div class="row pf">
            <div class="col-3">
            ...
            <div class="col-8">
              <input v-model="noTelp" type="text" class="form-control" id="NoTelpSupplier" name="noTelp" placeholder="Masukan No Telp Supplier" autofocus />
            </div>
          </div>

          <div class="d-flex justify-content-between pb">
            <router-link to="/indexsup">
...
export default {
  ...
  methods: {
    async getUpdateSupplier() {
      ...
      (this.namaSupplier = data.data.namaSupplier), (this.alamat = data.data.alamat), (this.noTelp = data.data.noTelp);
    ...
    async ChangeData() {
      ...
      await axios
        .put("http://159.223.57.121:8090/supplier/update/" + id, dataSupplier, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
            "Content-Type": "application/json",
          },
